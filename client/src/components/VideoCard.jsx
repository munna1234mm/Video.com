        ...
        <div className="relative">
            <Link to={`/video/${video.id}`} className="block">
                ...
                >
                    ...
                    {/* Info */}
                    <div className="flex gap-1 md:gap-3 items-start px-0.5">
                        <div className="hidden sm:block w-9 h-9 min-w-[36px] rounded-full overflow-hidden bg-gray-600">
                            {/* Avatar Placeholder */}
                            <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${video.uploader}`} alt={video.uploader} />
                        </div>
                        <div className="flex flex-col w-full overflow-hidden">
                            <div className="flex justify-between items-start gap-0.5">
                                <h3 className="text-white font-medium text-[8px] md:text-sm line-clamp-2 leading-tight group-hover:text-blue-400 transition-colors">
                                    {video.title}
                                </h3>
                                ...
                                >
                                    <MoreVertical className="text-white w-2.5 h-2.5 md:w-4 md:h-4" />
                                </button>
                            </div>
                            <div className="text-[#AAAAAA] text-[7px] md:text-xs mt-0.5 flex items-center gap-0.5">
                                <Link to={`/channel/${video.userId}`} className="hover:text-white transition-colors truncate">
                                    {video.uploader}
                                </Link>
                                <CheckCircle className="w-1.5 h-1.5 md:w-3 md:h-3 fill-[#AAAAAA] text-black" />
                            </div>
                            <div className="text-[#AAAAAA] text-[7px] md:text-xs">
                                {video.views} • {formatTimeAgo(video.uploadDate)}
                            </div>
                        </div>
                    </div>
                </div>
